...
class School:
    ...
    def remove_grade(self, name):
        idx = 0
        for i, grade in enumerate(self.grades): # enumerate returns the index and the value
            if grade.name == name:
                idx = i
        del self.grades[idx]
...
class Grade:
    ...
    def __repr__(self) -> str: # this is the string representation of the object
        return f'{self.name} with teacher {self.teacher}'

    def __del__(self): # this is the destructor of the object
